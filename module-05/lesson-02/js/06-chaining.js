...
// const greaterThenTwo = numbers.filter(val => val > 2);
// console.log(greaterThenTwo);

// const multByThree = greaterThenTwo.map(val => val * 3);
// console.log(multByThree);

// const sorted = multByThree.toSorted(
...
//   .filter(val => val > 2)
//   .map(val => val * 3)
//   .toSorted((next, current) => next - current);
...
const getSortedCarsOnSale = (cars) => {
  return cars.filter(car => car.onSale).toSorted(
    (next, current) => next.price - current.price
  )
};
